...
const AirQuality = (log, lat) => {
    fetch(
        `https://api.waqi.info/feed/geo:${lat};${log}/?token=${config.AIR_KEY}`
    )
        .then((res) => res.json())
        .then((res) => {
...
function generateWeatherItem(
    ...
    icon.alt = description.innerText;

    let dayTemp = document.createElement("div");
    dayTemp.innerHTML =
        `${languages[userLang].day} \t` + `${dayTemperature}&#176;C`;
    ...
    nightTemp.innerHTML =
        `${languages[userLang].night} \t` + `${nightTemperature}&#176;C`;
